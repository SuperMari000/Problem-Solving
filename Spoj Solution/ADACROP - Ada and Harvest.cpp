...
*/
	...
	int Found(Node *root,int v){
		if(root == empty)
			return -1e9;
		
		if(root->val > v)
			return Found(root->chld[L],v);
			
		int lsz = root->chld[L]->sz;
		if(root->val == v)
			return lsz;
			
		return Found(root->chld[R],v) + lsz + root->freq;
...
int main(){
	...
	while(m--){
		...
		cout<<avl.Lower_Bound(mp[b],a)<<endl;
		
		avl.Erase(mp[arr[a]],a);
		arr[a] = b;
		avl.Insert(mp[arr[a]],a);
